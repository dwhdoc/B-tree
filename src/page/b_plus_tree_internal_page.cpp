...
B_PLUS_TREE_INTERNAL_PAGE_TYPE::Lookup(const KeyType &key,const KeyComparator &comparator) const {
  ...
  while (S <= E) { //find the last key in array <= input
    int Middle = (E - S) / 2 + S;
    if (comparator(array[Middle].first,key) <= 0) S = Middle + 1;
    else E = Middle - 1;
  }
  return array[S - 1].second;
}
...
int B_PLUS_TREE_INTERNAL_PAGE_TYPE::InsertNodeAfter(const ValueType &old_value, const KeyType &new_key,const ValueType &new_value) {
  int Ind = ValueIndex(old_value) + 1;
  assert(Ind > 0);
  IncreaseSize(1);
  int CurSize = GetSize();
  ...
  array[Ind].first = new_key;
  array[Ind].second = new_value;
  return CurSize;
}
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::MoveHalfTo(
  ...
  assert(GetSize() == Tot);
  //copy last half
  int copyIdx = (Tot)/2;//max:4 x,1,2,3,4 -> 2,3,4
  page_id_t recipPageId = recipient->GetPageId();
  for (int i = copyIdx; i < Tot; i++) {
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::MoveFirstToEndOf(BPlusTreeInternalPage *recipient,BufferPoolManager *buffer_pool_manager) {
  ...
  Page *pg = buffer_pool_manager->FetchPage(childPageId);
  assert (pg != nullptr);
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::QueueUpChildren(std::queue<BPlusTreePage *> *queue,BufferPoolManager *buffer_pool_manager) {
  for (int a = 0; a < GetSize(); a++) {
    auto *page = buffer_pool_manager->FetchPage(array[a].second);
    if (page == nullptr)
