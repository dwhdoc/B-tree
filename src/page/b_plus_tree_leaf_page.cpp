...
int B_PLUS_TREE_LEAF_PAGE_TYPE::KeyIndex(
  ...
  assert(GetSize() >= 0);
  int S = 0, E = GetSize() - 1;
  while (S <= E) { //find the last key in array <= input
    int Middle = (E - S) / 2 + S;
    if (comparator(array[Middle].first,key) >= 0) E = Middle - 1;
    else S = Middle + 1;
  }
  return E + 1;
}
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
KeyType B_PLUS_TREE_LEAF_PAGE_TYPE::KeyAt(int Ind) const {
  assert(Ind >= 0 && Ind < GetSize());
  return array[Ind].first;
}
/*
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
const MappingType &B_PLUS_TREE_LEAF_PAGE_TYPE::GetItem(int Ind) {
  assert(Ind >= 0 && Ind < GetSize());
  return array[Ind];
}
...
int B_PLUS_TREE_LEAF_PAGE_TYPE::Insert(const KeyType &key,const ValueType &value,const KeyComparator &comparator) {
  int Ind = KeyIndex(key,comparator); //first larger than key
  assert(Ind >= 0);
  IncreaseSize(1);
  int CurSize = GetSize();

  for (int a = CurSize - 1; a > Ind; a--) {
    array[a].first = array[a - 1].first;
    array[a].second = array[a - 1].second;
  }
  array[Ind].first = key;
  array[Ind].second = value;
  return CurSize;
}
...
void B_PLUS_TREE_LEAF_PAGE_TYPE::MoveHalfTo(
  ...
  int Tot = GetMaxSize() + 1;
  assert(GetSize() == Tot);
  //copy last half
  int CopInd = (Tot)/2;//7 is 4,5,6,7; 8 is 4,5,6,7,8
  for (int a = CopInd; a < Tot; a++) {
    recipient->array[a - CopInd].first = array[a].first;
    recipient->array[a - CopInd].second = array[a].second;
  }
  //set pointer
...
void B_PLUS_TREE_LEAF_PAGE_TYPE::MoveAllTo(BPlusTreeLeafPage *recipient,
  ...
  //copy last half
  int BeginInd = recipient->GetSize();//7 is 4,5,6,7; 8 is 4,5,6,7,8
  for (int a = 0; a < GetSize(); a++) {
    recipient->array[BeginInd + a].first = array[a].first;
    recipient->array[BeginInd + a].second = array[a].second;
  }
  //set pointer
  recipient->SetNextPageId(GetNextPageId());
  //set size, is odd, bigger is last part
